...
export_general_log() {
	...
	if [ ".$isBetaUser" == ".1" ]; then
		...
		cp -r /data/vendor/ramdump/ssr_ramdump/ramdump* $GENERAL_LOG/ssr_ramdump/
		rm -r /data/vendor/ramdump/ssr_ramdump/ramdump*
		echo "cp -r /data/vendor/ramdump/ssr_ramdump/ramdump* $GENERAL_LOG/ssr_ramdump"
	else
...
export_nicmd_log(){
	cp -r $VENDOR_DATA_TMP/nicmd $GENERAL_LOG/
	chmod -R 777 $GENERAL_LOG/nicmd
	echo "/data/vendor/nicmd $ROOT_PATH/nicmd"
}
