...
if [ "${rawdump_enable}" = "1" ]; then
	...
	then
		...
		dd if=/dev/block/bootdevice/by-name/rawdump of=/temp/data/RawRamDump.bin
		echo "ASDF: save rawdump done." > /dev/kmsg
		if [ -f  $SAVE_DUMP_PATH/RawRamDump.tgz ]; then
			rm $SAVE_DUMP_PATH/RawRamDump.tgz
		fi
		tar -czvf $SAVE_DUMP_PATH/RawRamDump.tgz /temp/data/RawRamDump.bin
		echo "ASDF: tar dump DONE." > /dev/kmsg

		rm /temp/data/RawRamDump.bin
