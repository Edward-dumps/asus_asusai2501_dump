...
if [ "${factory_version}" = "1" ]; then
	...
	VENDOR_DATA_TMP=/data/temp
	mkdir -p $VENDOR_DATA_TMP 0777 root root encryption=None
	chcon u:object_r:ASDF_file:s0 /data/temp
fi
...
###########################################################################################
# Data
VENDOR_FOLDER=/data/vendor/nicmd
echo "$0: ++++ VENDOR_FOLDER=$VENDOR_FOLDER ++++"
chmod -R 777 $VENDOR_FOLDER
find $VENDOR_FOLDER -type f > $VENDOR_FOLDER/filelist.txt
LOG_LIST=$VENDOR_FOLDER/filelist.txt
# start log_mover
move_vendor_logs nicmd
echo "$0: cp -r /data/vendor/nicmd"
###########################################################################################
