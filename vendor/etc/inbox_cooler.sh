...
if [ "$csc_test_val" == "0" ] || [ "$csc_test_val" == "45" ]; then
	echo "[INBOX8_COOLER] CSC set cooler stage to $csc_test_val, skip command from AC" > /dev/kmsg
	exit
fi

if [  "$dongle_type" == "10" ]; then
	cooler_stage=`getprop persist.vendor.asus.coolerstage`
	echo "[INBOX8_COOLER] set cooler stage = $cooler_stage start" > /dev/kmsg
	echo $cooler_stage > /sys/class/leds/aura_inbox/cooling_stage
	echo "[INBOX8_COOLER] set cooler stage = $cooler_stage end" > /dev/kmsg
