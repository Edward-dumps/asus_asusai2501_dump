...
if [ "$dongle_type" == "10" -o "$dongle_type" == "9" ]; then
	...
	if [ "$cooler_stage_csc" == "-1" ]; then
		...
		echo "[ROG8_INBOX] CSC test end, recovery cooler stage to $cooler_stage" > /dev/kmsg
		...
		if [ "$cooler_stage_csc" == "0" ]; then
			...
			echo "[ROG8_INBOX][CSC] set cooler stage = $cooler_stage_csc" > /dev/kmsg
		fi
	fi
fi
